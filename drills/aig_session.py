...
class AIGSession:
    ...
    def __init__(self, params):
        ...
        self.nodes, self.level = float('inf'), float('inf')

        # self.best_known_nodes = (float('inf'), float('inf'), -1, -1)
        # self.best_known_level = (float('inf'), float('inf'), -1, -1)
        # self.best_known_nodes_meets_constraint = (float('inf'), float('inf'), -1, -1)
    ...
    def reset(self):
        ...
        self.log.write('iteration, optimization, nodes, levels\n')

        state, _ = self._run()

        # logging
        self.log.write(', '.join([str(self.iteration), self.sequence[-1], str(int(self.nodes)), str(int(self.level))]) + '\n')
        self.log.flush()
    ...
    def step(self, optimization):
        ...
        # logging
        # if self.nodes < self.best_known_nodes[0]:
        #     self.best_known_nodes = (int(self.nodes), int(self.level), self.episode, self.iteration)
        # if self.level < self.best_known_level[1]:
        #     self.best_known_level = (int(self.nodes), int(self.level), self.episode, self.iteration)
        # if self.level <= self.params['fpga_mapping']['levels'] and self.nodes < self.best_known_nodes_meets_constraint[0]:
        #     self.best_known_nodes_meets_constraint = (int(self.nodes), int(self.level), self.episode, self.iteration)
        self.log.write(', '.join([str(self.iteration), self.sequence[-1], str(int(self.nodes)), str(int(self.level))]) +'\n' )# + ', ' +
            # '; '.join(list(map(str, self.best_known_nodes_meets_constraint))) + ', ' + 
            # '; '.join(list(map(str, self.best_known_nodes))) + ', ' + 
            # '; '.join(list(map(str, self.best_known_level))) + '\n')
        self.log.flush()
    ...
    def _run(self):
        ...
        try:
            ...
            self.nodes, self.level = lut_6, levels
    ...
    def _get_reward(self, lut_6, levels):
        ...
        level_improvement = 0           # (-1, 0, 1) <=> (worse, same, improvement)

        # check optimizing parameter
        if lut_6 < self.nodes:
            optimization_improvement = 1
        elif lut_6 == self.nodes:
        ...
        if levels < self.level:
            ...
            level_improvement = -1
        return  optimization_improvement + level_improvement
